...
def _resolve_powerup_swap(board: Board, rng: random.Random, fr: dict, to: dict) -> dict:
    ...
    for pw in powerups:
        targets = powerup_targets(board, pw["pos"], pw["partner"])
    ...
    apply_gravity_and_refill(board, rng)
    chain = resolve_board(board, rng, fr, to)
...
def try_swap(board: Board, rng: random.Random, fr: dict, to: dict) -> dict:
    swap_cells(board, fr, to)
    power_res = _resolve_powerup_swap(board, rng, fr, to)
    if power_res["used_powerup"]:
        return power_res
    normal_res = resolve_board(board, rng, fr, to)
